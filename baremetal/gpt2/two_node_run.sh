	...
	for TUPLE in "${TUPLES[@]}"; do
		...
		for GRAD_ACCUM in 1; do
			...
			for RANK in 1 2 3 4 5 6 7 0; do
				...
				nsys profile --trace=cuda,nvtx,osrt --output /tmp/report.nsys-rep --force-overwrite true \
				torchrun \
				--nnodes=$NODES \
				--node-rank=$RANK \
				...
				if [ "$RANK" -eq 0 ]; then
					...
					ssh cloud-user@$NODEIP "ulimit -n 65535 && source llmc/bin/activate && env && nstat --reset > /dev/null && echo \"=======Writing nstate pre data=======\" && nstat && cd llm.c && NCCL_DEBUG=INFO NCCL_DEBUG_SUBSYS=NET $CMD && echo \"=======Writing nstate post data=======\" && nstat" >> $LOGFILE
				else
					ssh -f cloud-user@$NODEIP "ulimit -n 65535 && source llmc/bin/activate && cd llm.c && NCCL_DEBUG=INFO NCCL_DEBUG_SUBSYS=NET $CMD > /tmp/trainlog"  
				fi
			done
			sleep 5
			scp $N0:/tmp/report.nsys-rep ${LOGDIR}/${TYPE}.nsys-rep
			nsys stats ${LOGDIR}/${TYPE}.nsys-rep > ${LOGDIR}/${TYPE}_nsys_stats
		done
		done
	done
done
