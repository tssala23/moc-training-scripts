#!/bin/bash
declare -a TABLE=(
	"default via 192.168.50.254 dev eno2np0 proto dhcp src 192.168.50.151 metric 100" 
	"169.254.169.254 via 192.168.50.10 dev eno2np0 proto dhcp src 192.168.50.151 metric 100" 
	"192.168.50.0/24 dev eno2np0 proto kernel scope link src 192.168.50.151 metric 100"
	"192.168.60.103 dev eno8np0 scope link src 192.168.60.198" 
	"192.168.60.111 dev eno7np0 scope link src 192.168.60.173" 
	"192.168.60.185 dev eno6np0 scope link src 192.168.60.193"
	"192.168.60.186 dev eno5np0 scope link src 192.168.60.184"
)
